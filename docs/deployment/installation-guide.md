...
Give the client **one file: `install-lite.bat`**. They double-click it on the franchise computer, approve the Windows Administrator prompt, choose a strong first administrator password, and follow the Tailscale sign-in link. If the tailnet requires device approval, its administrator approves this computer and the client runs the BAT again. Keep the window open until it shows the private HTTPS address. Internet access is needed for GitHub, Python dependencies, Caddy, and Tailscale downloads.

The BAT installs Git and Python when missing, clones the current `main` branch into `C:\ProgramData\Setuora\Setuora-Lite`, installs hash-locked Python packages, creates a persistent local SQLite database, configures automatic local backups, and registers startup tasks for Lite and Caddy. It signs Tailscale in with unattended mode and configures Tailscale Serve on private HTTPS port 443. Lite and Caddy listen only on localhost; no LAN firewall rule or router port forwarding is required. Setup saves free local ports in `.env`; it preserves a free saved port on repair and chooses another when occupied. Master and Lite can share one computer; Tailscale Serve routes Master `/api/v1/` separately from Lite `/`.

Join every staff PC to the same tailnet and open the `https://<lite-name>.<tailnet>.ts.net` address shown by setup. Use that address for camera scanning. Staff still sign in with separate Lite user accounts. The Tailscale administrator controls which tailnet users can reach Lite. Lite's internal `http://127.0.0.1:<saved-port>` URL is for health checks and does not provide a staff browser login with secure cookies.
...
The menu also offers **Remove this installation**. It stops Lite, creates a recovery backup, and removes Lite's installed files and tasks while preserving the recovery copy. Master and its Tailscale route remain installed when both products share the computer.
